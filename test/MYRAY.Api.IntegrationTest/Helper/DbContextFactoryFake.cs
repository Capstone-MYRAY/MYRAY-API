...
public class DbContextFactoryFake : IDbContextFactory, IDisposable
{
    ...
    public DbContextFactoryFake(IConfiguration configuration)
    {
    ...
    public IUnitOfWork<TContext> GetContext<TContext>() where TContext : DbContext
    {
        ...
        if (!Contexts.ContainsKey(key))
        {
            ...
            TContext contextInstance = (TContext)Activator.CreateInstance(typeof(TContext),
                ConfigureSqlServer<TContext>(nameOfConectionString).Options);
